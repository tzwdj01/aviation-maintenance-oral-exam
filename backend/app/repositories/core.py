...
def execute_idempotently(
    ...
    existing = session.scalar(
        select(IdempotencyRecord).where(IdempotencyRecord.actor_id == actor_id, IdempotencyRecord.idempotency_key == key)
    )
    if existing:
    ...
    status, body = handler()
    session.add(IdempotencyRecord(actor_id=actor_id, idempotency_key=key, request_hash=digest, response_status=status, response_body=body))
...
def adopt_transcript(session: Session, transcript: ASRTranscript, actor_id: str) -> ASRTranscript:
    ...
    for sibling in session.scalars(select(ASRTranscript).where(ASRTranscript.answer_id == transcript.answer_id)):
...
def update_llm_profile_presentation(session: Session, profile: LLMProfile, *, display_name: str | None, enabled: bool | None, is_default: bool | None) -> LLMProfile:
