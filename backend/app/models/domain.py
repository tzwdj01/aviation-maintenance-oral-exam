...
class Answer(UUIDTimestampMixin, Base):
    ...
    audio_asset_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("media_assets.id"))


class MediaAsset(UUIDTimestampMixin, Base):
    """Audio artifact metadata; binary content lives in storage, never in the business table."""

    __tablename__ = "media_assets"
    __table_args__ = (UniqueConstraint("storage_key", name="uq_media_asset_storage_key"),)
    ...
    retention: Mapped[dict[str, Any]] = mapped_column(JSONType, default=dict, nullable=False)
    attempt_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("exam_attempts.id"))
    attempt_item_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("attempt_items.id"))
